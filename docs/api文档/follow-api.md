...
```javascript
// axios
axios.post('/api/user/follows/100', null, {
...
- 关注或恢复关注后，会同时清除关注者和被关注者两侧的关注计数缓存
...
| `40011` | 非法参数（如关注自己、目标用户已禁用） | 提示具体错误原因 |
| `40401` | 目标用户不存在或已删除 | 提示该用户不存在 |
...
|-----|------|---------|
| `401` | 未登录或 Token 失效 | 跳转登录页 |
| `40011` | 关注关系不存在或目标用户不可用 | 提示取消失败或已取消 |
| `40401` | 目标用户不存在或已删除 | 提示该用户不存在 |
...
|-----|------|---------|
| `401` | 未登录或 Token 失效 | 跳转登录页 |
| `40011` | 目标用户不可用（如关注自己、目标用户已禁用） | 提示用户信息无效 |
| `40401` | 目标用户不存在或已删除 | 提示用户信息无效 |
...
**统计规则**
- 实时 COUNT 统计有效关注关系（`followStatus = 1`）
- 不使用用户表冗余字段，每次实时计算
- 查询结果默认在 Redis 缓存 5 分钟，关注/取关/取关恢复操作会清除缓存
- 缓存读写失败不阻断业务，仅记录日志
...
|-----|------|---------|
| `401` | 未登录或 Token 失效 | 跳转登录页 |
| `40011` | 特别关注状态不合法、关注关系不存在或目标用户不可用 | 提示操作失败 |
| `40401` | 目标用户不存在或已删除 | 提示该用户不存在 |
...
| 字段 | 类型 | 必填 | 说明 |
|-----|------|-----|-----|
| `remark` | String | 否 | 备注内容，最大 256 字符，传空字符串或 `null` 清空备注 |
...
|-----|------|---------|
| `401` | 未登录或 Token 失效 | 跳转登录页 |
| `40011` | 备注超长、关注关系不存在或目标用户不可用 | 提示备注长度不能超过 256 字符或操作失败 |
| `40401` | 目标用户不存在或已删除 | 提示该用户不存在 |
...
| `40011` | `followStatus` 或 `specialFollow` 值不合法（非 `0/1`） | 提示参数错误 |
...
| `401` | 未登录或 Token 失效 | 跳转登录页 |
| `40300` | 无后台权限 | 提示没有访问权限 |
| `40001` | 清理条件不能为空或至少指定一个条件 | 提示至少选择一个清理条件 |
...
|-----|------|---------|---------|
| `200` | 成功 | 操作成功 | 正常流程 |
| `40001` | 参数校验失败 | 清理条件为空 | 提示具体校验错误 |
| `40011` | 非法参数 | 关注自己、特别关注状态不合法、备注超长、关注关系不存在、目标用户不可用 | 提示具体错误原因 |
| `401` | 未认证 | 未登录、Token 失效、Token 格式错误 | 跳转登录页 |
| `40300` | 没有访问权限 | 无后台权限 | 提示没有访问权限 |
| `40400` | 请求的接口不存在 | 路径错误 | 检查接口路径 |
| `40401` | 用户不存在 | 目标用户不存在或已删除 | 提示用户不存在 |
