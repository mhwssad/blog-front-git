  ...
  >
    ...
    <template v-else-if="detail">
      <div class="detail-header">
        <div class="detail-header__info">
          <div class="detail-header__name">
            <span v-if="detail.icon" class="detail-header__icon">{{ detail.icon }}</span>
            {{ detail.name }}
          </div>
          <div class="detail-header__sub">ID {{ detail.id }} · {{ formatMenuType(detail.type) }}</div>
        </div>
      ...
      <el-descriptions :column="2" border size="small" class="detail-section">
        ...
        <el-descriptions-item label="父菜单ID">{{ detail.parentId || '根菜单' }}</el-descriptions-item>
        <el-descriptions-item label="排序">{{ detail.sort }}</el-descriptions-item>
        <el-descriptions-item label="路由名称" :span="2">{{ detail.routeName || '-' }}</el-descriptions-item>
        <el-descriptions-item label="路由路径" :span="2">{{ detail.routePath || '-' }}</el-descriptions-item>
        <el-descriptions-item label="组件路径" :span="2">{{ detail.component || '-' }}</el-descriptions-item>
        <el-descriptions-item label="权限标识" :span="2">{{ detail.perm || '-' }}</el-descriptions-item>
        ...
        <el-descriptions-item label="显示">{{ detail.visible === 1 ? '显示' : '隐藏' }}</el-descriptions-item>
        <el-descriptions-item label="始终显示">{{ detail.alwaysShow === 1 ? '是' : '否' }}</el-descriptions-item>
        <el-descriptions-item label="缓存">{{ detail.keepAlive === 1 ? '是' : '否' }}</el-descriptions-item>
        <el-descriptions-item label="路由参数">{{ formatParams(detail.params) }}</el-descriptions-item>
      </el-descriptions>
      ...
      >
        <el-descriptions-item label="树路径">{{ detail.treePath }}</el-descriptions-item>
      </el-descriptions>
    </template>
