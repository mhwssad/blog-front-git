    ...
    <el-card class="search-card" shadow="never">
      <el-form :model="searchForm" inline class="search-form">
        <el-form-item label="模块名称" class="filter-item">
          <el-input v-model="searchForm.module" class="filter-control" clearable placeholder="请输入模块名称" />
        </el-form-item>
        <el-form-item label="操作类型" class="filter-item">
          <el-input v-model="searchForm.action" class="filter-control" clearable placeholder="请输入操作类型" />
        </el-form-item>
    ...
    <el-card class="table-card" shadow="never">
      ...
      <div ref="tableWrapperRef" class="table-wrapper">
        ...
        >
          ...
          <el-table-column prop="description" label="操作描述" min-width="220" align="center" show-overflow-tooltip />
          <el-table-column prop="username" label="操作人" min-width="120" align="center" show-overflow-tooltip />
          <el-table-column prop="ip" label="IP" min-width="120" align="center" show-overflow-tooltip />
          <el-table-column prop="requestMethod" label="请求方法" min-width="100" align="center" />
          <el-table-column prop="requestUrl" label="请求地址" min-width="220" align="center" show-overflow-tooltip />
          ...
          >
            <template #default="{ row }">
              <div class="table-actions" :class="{ 'table-actions--compact': isCompactTable }">
                <el-button v-permission="'sys:log:query'" link type="primary" @click="handleViewDetail(row)">
                  详情
                </el-button>
                <el-button v-permission="'sys:log:delete'" link type="danger" @click="handleDelete(row)">
                  删除
                </el-button>
              </div>
            </template>
...
async function handleClean(): Promise<void> {
  ...
  try {
    await ElMessageBox.confirm('确定要按当前时间范围清理日志吗？此操作不可恢复。', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
